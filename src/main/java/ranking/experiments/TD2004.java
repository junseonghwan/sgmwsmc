...
public class TD2004 implements Runnable
{
	@Option
	public static String dataLoc = "data/ranking/TD2004/QueryLevelNorm/";
	@Option
	public static String outputPath = "output/ranking/TD2004/Scores/";
	@Option
	...
	public void run()
	{
		...
		Random random = new Random(20160513);
		for (int fold : folds)
		{
	...
	public void runExperiment(Random random, int fold)
	{
		...
		double [] lambdas = {0.25, 0.5, 0.75, 1, 1.25, 1.5};
		//double [] lambdas = {1};
		...
		for (double lambda : lambdas)
		{
			...
			double [] params = train(new Random(random.nextLong()), data, startingPoint, lambda, B, L, fold);
			// predict on the validation set
			double eval = test(fold, params, validationFile, false)[k-1];
	...
	public static double [] DCG(List<Document> documents)
	{
		...
		for (int i = 1; i < k; i++)
		{
			...
			if (i < 2) {
				...
				DCG[i] = DCG[i-1] + (rel * Math.log(2.0)/Math.log(i + 1));
			}
		}
		return DCG;
	}
	...
	{
		Mains.instrumentedRun(args, new TD2004());
	}
