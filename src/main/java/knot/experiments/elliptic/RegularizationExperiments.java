...
public class RegularizationExperiments implements Runnable 
{
	@Option public static Random rand = new Random(1);
	@Option public static String [] dataDirectories = {"data/16Mar2016/", "data/21Oct2015/"};
	...
	public static double lmin = 2;
	public static double lmax = 21;
	public static double interval = 2;
	@Option public static int K = 5;
	@Option public static int numRep = 20;
	...
	{
		// read the data
		List<List<Segment>> instances = KnotExpUtils.readSegmentedBoard(inputFileName, BOARDS, false);
		...
		// output to file
		PrintWriter writer = BriefIO.output(new File(outputPath + "segmented_l2_regularization.csv"));
		...
		writer = BriefIO.output(new File(outputPath + "segmented_l2_regularization_details.csv"));
		...
		writer = BriefIO.output(new File(outputPath + "segmented_l2_regularization_params.csv"));
	...
	{
		Mains.instrumentedRun(args, new RegularizationExperiments());
	}
